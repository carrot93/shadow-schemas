addShadow({
  ...
  after: function (collection) {
    ...
    collection.insert = _.wrap(collection.insert, function () {
      var func =  Array.prototype.shift.apply(arguments);
      if (!collection._instanceField) { //no schema don't validate 
    ...
    collection.update = _.wrap(collection.update, function () {
      var func =  Array.prototype.shift.apply(arguments);
      if (!collection._instanceField) { //no schema don't validate
        return func.apply(collection, arguments);
      }
      
      var modifier = arguments[1], options = arguments[2] || {};
      
      if (options.multi) {
        var docs = collection.find(arguments[0]).fetch();
      } else { //be efficient but put in an array to keep things simple
        var docs = [collection.findOne(arguments[0])];
      }
